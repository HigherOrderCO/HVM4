...
C_BIN="$ROOT_DIR/src/hvm"
C_MAIN="$ROOT_DIR/src/hvm.c"
...
EOF
    ...
    *)
      ...
      echo "run devs/test/_all_.sh --help for usage" >&2
      exit 1
      ;;
  esac
  shift
...
status=0
if [ -n "${HVM_TEST_FLAGS:-}" ]; then
  run_tests "$C_BIN" "HVM" "$TEST_TIMEOUT_INTERPRETED_SECS" "${shared_flags[@]}"
else
  run_tests "$C_BIN" "HVM" "$TEST_TIMEOUT_INTERPRETED_SECS"
fi
if [ $? -ne 0 ]; then
  status=1
fi
