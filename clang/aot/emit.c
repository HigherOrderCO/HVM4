  ...
  if (head) {
    ...
    switch (tag) {
      ...
      case MAT: {
        ...
        aot_emit_tmp(dp_ctr, sizeof(dp_ctr), "dp_ctr", tmp);
        ...
        aot_emit_itrs_inc(f, pad1);
        fprintf(f, "%sfor (u32 j = %s; j > 0; j--) {\n", pad1, ari);
        fprintf(f, "%s  Term %s = term_new(0, %s == 0 ? DP0 : DP1, %s, %s + (u64)(j - 1));\n", pad1, fld, dp_side, dp_lab, dp_ctr);
        ...
        fprintf(f, "%s  aot_deopt_site_observe_term(%u, %s);\n", pad1, site, arg);
        fprintf(f, "%s  return aot_fallback_alo(%lluULL, %u, ", pad1, (unsigned long long)loc, dep);
        aot_emit_env_head(f, dep);
        fprintf(f, ", %u, %u);\n", AOT_DEOPT_EXPR_UNSUPPORTED_TAG, site);
        fprintf(f, "%s}\n", pad1);
        fprintf(f, "%sif (term_ext(%s) == %u) {\n", pad1, arg, term_ext(term));
        fprintf(f, "%su32 %s = (u32)(%s - C00);\n", pad2, ari, tag_n);
        fprintf(f, "%sif (%s) {\n", pad2, src);
        fprintf(f, "%sif ((u64)(*a_pos) + (u64)%s > (u64)AOT_ARG_CAP + 1ULL) {\n", pad, ari);
        aot_emit_ret_fallback_loc(f, loc, dep, AOT_DEOPT_HEAD_APP_ARG_CAP, pad);
        fprintf(f, "%s}\n", pad);
        fprintf(f, "%s(*a_pos)--;\n", pad);
        fprintf(f, "%s} else {\n", pad2);
        fprintf(f, "%sif ((u64)(*a_pos) + (u64)%s > (u64)AOT_ARG_CAP) {\n", pad, ari);
        ...
        fprintf(f, "%s  (*a_pos)++;\n", pad2);
        fprintf(f, "%s}\n", pad2);
        aot_emit_node(f, mat_loc + 0, dep, out, 1, pad2, tmp);
        fprintf(f, "%s} else {\n", pad1);
        aot_emit_itrs_inc(f, pad2);
        aot_emit_node(f, mat_loc + 1, dep, out, 1, pad2, tmp);
        fprintf(f, "%s}\n", pad1);
        fprintf(f, "%s}\n", pad);
        return;
      }
