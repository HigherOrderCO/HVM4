// %tcp_recv_poll(tcp, max_bytes)
// ------------------------------
// %tcp_recv_poll_go_tcp(tcp, max_bytes)
fn Term prim_fn_tcp_recv_poll(Term *args) {
  Term args0[2] = {args[0], args[1]};
  Term t = term_new_pri(table_find("tcp_recv_poll_go_tcp", sizeof("tcp_recv_poll_go_tcp") - 1), 2, args0);
  return wnf(t);
}
...
fn Term tcp_recv_poll_go_tcp(Term *args) {
  Term tcp_wnf   = wnf(args[0]);
  Term max_bytes = args[1];

  switch (term_tag(tcp_wnf)) {
    case ERA: {
      // %tcp_recv_poll_go_tcp(&{}, max_bytes)
      // ------------------------------------- tcp-recv-poll-go-tcp-era
      // &{}
      return term_new_era();
    }
    case INC: {
      // %tcp_recv_poll_go_tcp(↑x, max_bytes)
      // ------------------------------------ tcp-recv-poll-go-tcp-inc
      // ↑(%tcp_recv_poll(x, max_bytes))
      u32  inc_loc = term_val(tcp_wnf);
      Term inner   = heap_read(inc_loc);
      Term args0[2] = {inner, max_bytes};
      Term next    = term_new_pri(table_find("tcp_recv_poll", sizeof("tcp_recv_poll") - 1), 2, args0);
      heap_set(inc_loc, next);
      return term_new(0, INC, 0, inc_loc);
    }
    case SUP: {
      // %tcp_recv_poll_go_tcp(&L{x,y}, max_bytes)
      // ----------------------------------------- tcp-recv-poll-go-tcp-sup
      // &L{%tcp_recv_poll(x, max0), %tcp_recv_poll(y, max1)}
      ...
      Term a0[2]   = {x, M.k0};
      Term a1[2]   = {y, M.k1};
      Term t0      = term_new_pri(table_find("tcp_recv_poll", sizeof("tcp_recv_poll") - 1), 2, a0);
      Term t1      = term_new_pri(table_find("tcp_recv_poll", sizeof("tcp_recv_poll") - 1), 2, a1);
      return term_new_sup(lab, t0, t1);
    }
    default: {
      // %tcp_recv_poll_go_tcp(tcp, max_bytes)
      // ------------------------------------- tcp-recv-poll-go-tcp-default
      // %tcp_recv_poll_go_max(tcp, max_bytes)
      Term args0[2] = {tcp_wnf, max_bytes};
      Term t = term_new_pri(table_find("tcp_recv_poll_go_max", sizeof("tcp_recv_poll_go_max") - 1), 2, args0);
      return wnf(t);
    }
  }
}
...
fn Term tcp_recv_poll_go_max(Term *args) {
  Term tcp       = args[0];
  Term max_wnf   = wnf(args[1]);

  switch (term_tag(max_wnf)) {
    case ERA: {
      // %tcp_recv_poll_go_max(tcp, &{})
      // ------------------------------- tcp-recv-poll-go-max-era
      // &{}
      return term_new_era();
    }
    case INC: {
      // %tcp_recv_poll_go_max(tcp, ↑x)
      // ------------------------------ tcp-recv-poll-go-max-inc
      // ↑(%tcp_recv_poll(tcp, x))
    ...
    case SUP: {
      // %tcp_recv_poll_go_max(tcp, &L{x,y})
      // ------------------------------------ tcp-recv-poll-go-max-sup
      // &L{%tcp_recv_poll(tcp0, x), %tcp_recv_poll(tcp1, y)}
    ...
    default: {
      // %tcp_recv_poll_go_max(tcp, max_bytes)
      // ------------------------------------- tcp-recv-poll-go-max-default
      // %tcp_recv_poll_go_io(tcp, max_bytes)
      Term args0[2] = {tcp, max_wnf};
      Term t = term_new_pri(table_find("tcp_recv_poll_go_io", sizeof("tcp_recv_poll_go_io") - 1), 2, args0);
      return wnf(t);
    }
  }
}
...
fn Term prim_fn_tcp_recv_poll_go_io(Term *args) {
  ...
  if (!tcp_parse_handle(args[0], &id, &seq)) {
    return tcp_new_err("tcp_recv_poll", TCP_ERR_BAD_HANDLE, "invalid `tcp`; expected #Tcp{id,seq}");
  }

  if (!tcp_is_valid_id(id)) {
    return tcp_new_err("tcp_recv_poll", TCP_ERR_BAD_HANDLE, "unknown tcp id");
  }
  ...
  if (!tcp_claim(id, seq, &snap)) {
    return tcp_new_err("tcp_recv_poll", TCP_ERR_STALE, "stale tcp handle");
  }
  ...
  switch (snap.state) {
    ...
    case TCP_STATE_OPEN: {
      ...
      if (!buf) {
        return tcp_new_err("tcp_recv_poll", TCP_ERR_IO, "out of memory");
      }
...
fn void prim_tcp_recv_poll_init(void) {
  prim_register("tcp_recv_poll", sizeof("tcp_recv_poll") - 1, 2, prim_fn_tcp_recv_poll);
  prim_register("tcp_recv_poll_go_tcp", sizeof("tcp_recv_poll_go_tcp") - 1, 2, tcp_recv_poll_go_tcp);
  prim_register("tcp_recv_poll_go_max", sizeof("tcp_recv_poll_go_max") - 1, 2, tcp_recv_poll_go_max);
  prim_register("tcp_recv_poll_go_io", sizeof("tcp_recv_poll_go_io") - 1, 2, prim_fn_tcp_recv_poll_go_io);
}
