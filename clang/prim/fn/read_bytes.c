...
fn Term prim_fn_read_bytes_go_io(Term *args) {
  ...
  Term h_t[2] = {term_new_ctr(SYM_BYT, 1, byt), Nil};
  Term result = term_new_ctr(SYM_CON, 2, h_t);

  // `curr` is the last #CON in the output List<#BYT{NUM}>.
  Term curr = result;
  while (fread(&c, 1, 1, file) == 1) {
    byt[0] = term_new_num(c);
    h_t[0] = term_new_ctr(SYM_BYT, 1, byt);
    // Append #CON{#BYT{NUM}, #NIL} at curr tail.
    heap_set(term_val(curr) + 1, term_new_ctr(SYM_CON, 2, h_t));
